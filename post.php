...
// 댓글 목록 가져오기
$stmt = $pdo->prepare("SELECT comments.*, users.username, users.homepage, users.profile_image FROM comments JOIN users ON comments.user_id = users.id WHERE comments.post_id = ? ORDER BY comments.created_at ASC");
$stmt->execute([$post_id]);
            ...
            <?php if (!empty($files)): ?>
                ...
                <ul>
                    <?php foreach ($files as $file): ?>
                        <li><a href="download.php?file=<?php echo basename($file['file_path']) ;?>"><?php echo htmlspecialchars($file['file_name']); ?></a></li>
                    <?php endforeach; ?>
                </ul>
            <?php endif; ?>
